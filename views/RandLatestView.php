<?php
class RandLatestView
{
	public function index($dicts, $marked, $rand_word, $num, $round=0, $count=0, $reverse=FALSE, $pron=FALSE, $pron_us=FALSE, $pron_uk=FALSE)
	{
...
<body>
	...
	<div class="content">
		...
		<div class="main">
			...
			?>
			<form action="<?=$reverse?'/rand/latest/reverse/'.$num:'/rand/latest/'.$num?>" method="post">
...
<?php
	...
	protected function pron($rand_word, $pron_us=FALSE, $pron_uk=FALSE)
	{
		...
		if($this->url_exists($ref))
		{
			...
			$file = $page->find('span.pron-info>span.us:eq(0)>span:eq(1)')->attr('data-src-mp3');
			$file = "https://dictionary.cambridge.org".$file;
			$trans = $page->find('span.pron-info>span.us:eq(1)>span.pron>span.ipa')->text();
			echo '<p*                                               *p>';
			echo '<audio controls>';
			...
			//preg_match('~<span class="ipa">(.+?)</span>~', $x, $trans);
			$file = $page->find('span.pron-info>span.uk:eq(0)>span:eq(1)')->attr('data-src-mp3');
			$file = "https://dictionary.cambridge.org".$file;
			$trans = $page->find('span.pron-info>span.uk:eq(1)>span.pron>span.ipa')->text();
			echo '<audio controls>';
				echo '<source src="'.$file.'" type="audio/mpeg">';
				echo 'Your browser does not support the audio element.data-src-mp3';
